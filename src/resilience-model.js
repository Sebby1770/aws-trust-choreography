/**
 * Pure resilience domain model for the AWS Command Atlas.
 *
 * This module holds the scenario data and the composition logic that turns a
 * scenario plus a set of injected faults into derived telemetry. It has no DOM
 * dependencies so it can be unit-tested in isolation (see tests/).
 */
...
export const failureModes = {
  edge: {
    ...
    recoveryMinutes: 5,
    recommendation:
      "Rate-limit at the edge, preserve cache capacity, and scale compute only after request quality is visible.",
  },
  identity: {
    id: "identity",
    ...
    recoveryMinutes: 8,
    recommendation:
      "Freeze role changes, route elevation through manual approval, and diff trust relationships before restoring speed.",
  },
  data: {
    id: "data",
    label: "Data lag",
    ...
    recoveryMinutes: 10,
    recommendation:
      "Throttle writes, protect the last known recovery point, and prove replay order before reopening mutation paths.",
  },
  workflow: {
    id: "workflow",
    ...
    recoveryMinutes: 7,
    recommendation:
      "Quarantine poison work, cap retries, and drain one compensation lane at a time with evidence attached.",
  },
};
    ...
    cards: [
      {
        ...
        detail:
          "The quiet risk is not outage, but losing track of which controls still explain the system.",
      },
      {
      ...
      },
      {
        ...
        value: "Fresh replay",
        detail:
          "A recent Step Functions replay plus S3 evidence snapshot proves the map is trustworthy.",
      },
    ],
  },
  surge: {
    ...
    cards: [
      {
        ...
        detail:
          "A cache miss wave can turn compute, data, and identity into one coupled bottleneck.",
      },
      {
        label: "Operating stance",
        value: "Absorb first",
      ...
      },
      {
        ...
        detail:
          "EventBridge should preserve the shape of demand so autoscaling does not erase the lesson.",
      },
    ],
  },
  identity: {
...
export const nodeHints = {
  ...
  "IAM trust broker":
    "Federated identity contracts are reviewed before emergency permissions widen.",
  "Aurora data mesh": "Read replicas and event streams keep product state moving through recovery.",
  "Step Functions core":
    "Workflow state machine coordinates retries, compensations, and audit events.",
...
export const nodeLinks = {
  "CloudFront edge": { href: "https://aws.amazon.com/cloudfront/", label: "AWS CloudFront" },
  "EKS compute fleet": { href: "https://aws.amazon.com/eks/", label: "Amazon EKS" },
  "IAM trust broker": { href: "https://aws.amazon.com/iam/", label: "AWS IAM" },
  "Aurora data mesh": { href: "https://aws.amazon.com/rds/aurora/", label: "Amazon Aurora" },
  "Step Functions core": {
    href: "https://aws.amazon.com/step-functions/",
    label: "AWS Step Functions",
  },
  "Manual approval lane": { href: "https://aws.amazon.com/sns/", label: "Amazon SNS" },
  "Lambda decision agent": { href: "https://aws.amazon.com/lambda/", label: "AWS Lambda" },
  "S3 evidence vault": { href: "https://aws.amazon.com/s3/", label: "Amazon S3" },
  "EventBridge mesh": { href: "https://aws.amazon.com/eventbridge/", label: "Amazon EventBridge" },
};
...
/**
 * Compose a scenario with a set of injected faults into derived telemetry.
 *
...
export function deriveComposedState(name, activeFailures = [], scoreOverrides = {}) {
  ...
  const activeModeObjects = Array.from(activeFailures)
    .map((id) => failureModes[id])
    .filter(Boolean);
  ...
  const summary = activeModeObjects.length
    ? `${activeModeObjects.length} fault${activeModeObjects.length === 1 ? "" : "s"} injected: ${activeModeObjects
        .map((mode) => mode.label)
        .join(" + ")}`
    : "No faults injected";
